...
const FeaturedProperty = () => {
  ...
  const PropertyData = productItem.filter((item) => item.type === "property").slice(0, 4);
  return (
    <div className="featured-info-box">
      <h4 className="detail-page-title">{FeaturedPropertyTitle}</h4>
      <ul className="featured-detail-box">
        {PropertyData.map((item, index) => (
          <li key={index}>
            <Link to={RouteList.Property.Detail.PropertySidebarLayout} className="featured-detail-item">
